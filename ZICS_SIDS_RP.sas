...
* LOCATION			: \\ad.bu.edu\bumcfiles\SPH\DCC\Dept\ZICS\09SAS_Programs\06Analysis\Secondary_Analyses\20240522_SIDS
* DESCRIPTION   	: program the descriptive table for SIDS
* SPEC NAME			: Specifications_SIDS_Paper_20240620
* SPEC LOC			: \\ad.bu.edu\bumcfiles\SPH\DCC\Dept\ZICS\09SAS_Programs\06Analysis\Secondary_Analyses\20240522_SIDS
* PROGRAMMER    	: Janaki Shah
* DATE WRITTEN  	: 2024.06.21
* GENERAL NOTES		: 
****************************************************************************************
* NOTES FOR QC		: 
* QC PROGRAMMER		: Leah Forman
* QC DATE			: 2024.07.01
* QC STATUS			: Complete
* QC NOTES			: 
****************************************************************************************
...
/*LSF 2024.07.03 - added REDCAP_EVENT_NAME to this KEEP statement*/
...
/*LSF 2024.06.28 - 
	You have to sort here by all of the variables that you kept in TABS2 
	If you sort just by INFANTID - you keep only the first one for each INFANTID. We dont necessarily 
	want to do that here, so you have to sort by all of the variables that you kept above, and then 
	we have to look into the ones where there is more than one in the resulting table.
	SO ... you should: 

	1. update the PROC SORT to sort by all of the variables in the SELECT statement above
	2. Run the QC statements again to see how many times each INFANTID appears 
	3. For this program, for the ones that appear more than once - keep their 20 week visit over their 24 week visit: 
	4. New datastep and code to write for the IDs that appear more than once in the dataset after you update 
	the PROC SORT and determine which IDs those are: 

	if infantid = XXXX.X and redcap_event_name = 'w24_arm_1' then delete; 
...
1. Create a dataset from ZICS_POSTNATAL that just has: infantid redcap_event_name vac_bf_dv vac_bf_dv5 
2. Merge this dataset with TABLE2E by INFANTID REDCAP_EVENT_NAME, and limit to observations in TABLE2E


Then, you can add VAC_BF_DV and VAC_BF_DV5 to the final table output
...
outfile=\\ad.bu.edu\bumcfiles\SPH\DCC\Dept\ZICS\09SAS_Programs\06Analysis\Secondary_Analyses\20240522_SIDS\SIDS_table1_&rundate..rtf,excluvar=studyid infantid marital2 marstat educ_mob tob_preg,dispp=0);
...
outfile=\\ad.bu.edu\bumcfiles\SPH\DCC\Dept\ZICS\09SAS_Programs\06Analysis\Secondary_Analyses\20240522_SIDS\SIDS_table1b_&rundate..rtf,excluvar=studyid infantid PREM_DELIVERY PREM_DELIVERY37 PREM_DELIVERY34 bwcat BW  BRTHANOMSP vac_bf_dv ,dispp=0);
...
outfile=\\ad.bu.edu\bumcfiles\SPH\DCC\Dept\ZICS\09SAS_Programs\06Analysis\Secondary_Analyses\20240522_SIDS\SIDS_table2_&rundate..rtf,excluvar=studyid infantid REDCAP_EVENT_NAME ,dispp=0);
...
/**************************************/
ods rtf file = "\\ad.bu.edu\bumcfiles\SPH\DCC\Dept\ZICS\09SAS_Programs\06Analysis\Secondary_Analyses\20240522_SIDS\SIDSresults_&rundate..rtf";
